...
Laser::Laser(int laserPinR, int laserPinG, int laserPinB)
{
  _laserPinR = laserPinR;
  _laserPinG = laserPinG;
  _laserPinB = laserPinB;

  _laserBrightnessR = 255;
  _laserBrightnessG = 255;
  _laserBrightnessB = 255;
...
void Laser::init()
{
  ...
  pinMode(_laserPinR, OUTPUT);
  pinMode(_laserPinG, OUTPUT);
  pinMode(_laserPinB, OUTPUT);
}
...
void Laser::drawline(long x1, long y1, long x2, long y2)
{
  ...
  on();
  sendto(x2,y2);
  //wait(LASER_LINE_END_DELAY); DK replace - this was designed for chaining lines together
  off();
}
...
void Laser::on()
{
  if (!_state && !_laserForceOff) 
  {
    ...
    _state = 1;
    analogWrite(_laserPinR, _laserColorR ? _laserBrightnessR : 0);
    analogWrite(_laserPinG, _laserColorG ? _laserBrightnessG : 0);
    analogWrite(_laserPinB, _laserColorB ? _laserBrightnessB : 0);
  }
}
...
void Laser::off()
{
  if (_state) 
  {
    wait(LASER_TOGGLE_DELAY);
    _state = 0;
    digitalWrite(_laserPinR, LOW);
    digitalWrite(_laserPinG, LOW);
    digitalWrite(_laserPinB, LOW);
  }
}
...
void Laser::setColor(bool red, bool green, bool blue)
{
  _laserColorR = red;
  _laserColorG = green;
  _laserColorB = blue;
...
void Laser::setPWM(byte red, byte green, byte blue)
{
  _laserBrightnessR = red;
  _laserBrightnessG = green;
  _laserBrightnessB = blue;
}
