...
// DK Add
void Drawing::drawLineRotated(unsigned short posX1, unsigned short posY1, unsigned short posX2, unsigned short posY2, long centerX, long centerY, int angle, bool turnOffLaserWhenDone)
{
  FIXPT x = (long)(posX1 & 0x7fff) - centerX;
  FIXPT y = ((long)posY1) - centerY;
  FIXPT x1 = COS(angle) * x - SIN(angle) * y;
  FIXPT y1 = COS(angle) * y + SIN(angle) * x;
  laser.sendto(TO_INT(x1), TO_INT(y1));
  ...
  x = (long)(posX2 & 0x7fff) - centerX;
  y = ((long)posY2) - centerY;
  x1 = COS(angle) * x - SIN(angle) * y;
  y1 = COS(angle) * y + SIN(angle) * x;
  laser.sendto(TO_INT(x1), TO_INT(y1));

  if(turnOffLaserWhenDone) {
    laser.off();
  }
}

// DK Add
void Drawing::drawDotRotated(unsigned short posX1, unsigned short posY1, unsigned short strength, long centerX, long centerY, int angle)
{
  FIXPT x = (long)(posX1 & 0x7fff) - centerX;
  FIXPT y = ((long)posY1) - centerY;
  FIXPT x1 = COS(angle) * x - SIN(angle) * y;
